...
/**
 * 본문 렌더러 — 본문 HTML에 계산기 마커가 있으면 실제 계산기를 그 위치에 임베드한다.
 * 마커: <!--CALC:didimdol--> 또는 <!--CALC:cheongyak-->
 * "읽는 콘텐츠"를 "직접 써보는 콘텐츠"로 전환해 페이지 고유 가치를 높인다.
 */
function ArticleBody({ html, onNavigate }: { html: string; onNavigate: (p: string) => void }) {
  const CALC_RE = /<!--CALC:(didimdol|cheongyak)-->/g;
  if (!CALC_RE.test(html)) {
    return <div className="article-body" dangerouslySetInnerHTML={{ __html: html }} />;
  }
  // 마커 기준으로 분할: [html, type, html, type, ...]
  const parts = html.split(/<!--CALC:(didimdol|cheongyak)-->/);
  const nodes: React.ReactNode[] = [];
  for (let i = 0; i < parts.length; i++) {
    if (i % 2 === 0) {
      if (parts[i].trim())
        nodes.push(
          <div key={`h${i}`} className="article-body" dangerouslySetInnerHTML={{ __html: parts[i] }} />
        );
    } else {
      const type = parts[i];
      nodes.push(
        <div key={`c${i}`} className="my-10 -mx-1 sm:mx-0 rounded-2xl border border-[#E2E4F0] bg-white overflow-hidden">
          <div className="bg-[#312E81] px-5 py-3 flex items-center gap-2">
            <span className="text-[13px] font-bold text-white">🧮 이 글의 내용을 직접 계산해보세요</span>
          </div>
          <div className="p-1 sm:p-2">
            {type === "didimdol" ? <DidimdolCalculator /> : <CheongyakCalculator />}
          </div>
          <div className="px-5 py-3 border-t border-[#EDEEF7] bg-[#FAFBFF]">
            <button
              onClick={() => onNavigate(type === "didimdol" ? "tools-didimdol" : "tools-cheongyak")}
              className="text-[12.5px] font-medium text-[#4F46E5] hover:text-[#3730A3]"
            >
              계산기 전체 화면으로 보기 →
            </button>
          </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {currentPage === "about" && (
            ...
            >
              ...
              <ul>
                ...
                <li><strong>사이트:</strong> <a href="https://virginroad.kr">https://virginroad.kr</a></li>
              </ul>
            </motion.div>
          )}
          ...
          {currentPage === "privacy" && (
            ...
            >
              ...
              <ul>
                <li><a href="https://adssettings.google.com">Google 광고 설정 페이지</a>에서 맞춤 광고 비활성화</li>
                <li><a href="https://policies.google.com/technologies/ads">Google 광고 정책</a> 확인</li>
                <li><a href="https://policies.google.com/privacy">Google 개인정보처리방침</a> 확인</li>
              </ul>
              ...
              <ul>
                <li><strong>개인정보분쟁조정위원회:</strong> (국번 없이) 1833-6972 / <a href="https://www.kopico.go.kr">www.kopico.go.kr</a></li>
                <li><strong>개인정보침해 신고센터:</strong> (국번 없이) 118 / <a href="https://privacy.kisa.or.kr">privacy.kisa.or.kr</a></li>
                <li><strong>대검찰청 사이버수사과:</strong> (국번 없이) 1301 / <a href="https://www.spo.go.kr">www.spo.go.kr</a></li>
                <li><strong>경찰청 사이버수사국:</strong> (국번 없이) 182 / <a href="https://ecrm.cyber.go.kr">ecrm.cyber.go.kr</a></li>
              </ul>
          ...
          {currentPost ? (
            ...
            >
              ...
              {/* Article body — 계산기 마커가 있으면 실제 계산기를 본문에 임베드 */}
              <ArticleBody
